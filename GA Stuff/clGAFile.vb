...
Public Class clGAFile
    ...
    'GA Visitor Data
    Private _ColumnsV As Hashtable = New Hashtable()
    Private _ColTypesV As Hashtable = New Hashtable()
...
#Region " Sub New "
    ...
    'Will create two HashTable objects from settings.xml/oSettings.GAColumns
    'One for Column names, _Columns, and another for Column Types, _ColTypes
    '================================================================================
    Sub New(ByVal oSettings As clSettings)
        Files = oSettings.SaveBaseLocation & "GA\"
        ConnectionString = oSettings.ConnectionString
        DirectoryNamesAsRecID = oSettings.DirectoryNamesAsRecID

        Dim nCount As Integer
        _Columns.Clear()
        For nCount = oSettings.GAColumns.GetLowerBound(0) To oSettings.GAColumns.GetUpperBound(0)
            If Not IsNothing(oSettings.GAColumns(nCount)) Then
                _Columns.Add(oSettings.GAColumns(nCount).Name, oSettings.GAColumns(nCount).Value)
            End If
        Next
        ...
        'GA Visitor Data
        _ColumnsV.Clear()
        For nCount = oSettings.GAVColumns.GetLowerBound(0) To oSettings.GAVColumns.GetUpperBound(0)
            If Not IsNothing(oSettings.GAVColumns(nCount)) Then
                _ColumnsV.Add(oSettings.GAVColumns(nCount).Name, oSettings.GAVColumns(nCount).Value)
            End If
        Next

        _ColTypesV.Clear()
        For nCount = oSettings.ColumnTypesV.GetLowerBound(0) To oSettings.ColumnTypesV.GetUpperBound(0)
            If Not IsNothing(oSettings.ColumnTypesV(nCount)) Then
                _ColTypesV.Add(oSettings.ColumnTypesV(nCount).Name.ToLower, oSettings.ColumnTypesV(nCount).Value)
            End If
        Next
...
#Region " Process CSV File "
    ...
    Private Function Processfile_CSV(ByVal sFile As String, ByVal oClient As Clients, ByVal FileDate As Date) As Boolean
        ...
        Dim sGAAcctID As String = oClient.GA.AccountID
            ...
            If sLine.Length > 0 Then
                ...
                If oDB.IsdataValid Then
                    If nColumnDate >= 0 Then
                        oDB.myDate = Date.ParseExact(oDB.Data(nColumnDate), "yyyyMMdd", Globalization.CultureInfo.InvariantCulture)
                        'oDB.myDate = CDate(oDB.Data(nColumnDate))
                    End If
                    oDB.EngineID = oDB.Data(1)
                    oDB.UpdateSQLGA(oClient.IsArbitrage, cmd)
                    Application.DoEvents()
                End If
    ...
    End Function
    '================================================================================
    'ProcessfileV_CSV(): Process file from API and load into DB                      
    '================================================================================
    Private Function ProcessfileV_CSV(ByVal sFile As String, ByVal oClient As Clients, ByVal FileDate As Date) As Boolean

        Debug.Print("Processing GA Visitor CSV")
        Dim sGAAcctID As String = oClient.GA.AccountID
        ...
        'Mappings for Column names used in API build file download to the DB column names
        oDB.Columns = ParseColumnsV(sHeader)
        oDB.DataTypes = _ColTypesV
        ...
        While Not fs.EndOfStream
            ...
            If sLine.Length > 0 Then
                ...
                If oDB.IsdataValid Then
                    ...
                    End If
                    oDB.EngineID = 0
                    oDB.UpdateSQLGAV(oClient.IsArbitrage, cmd)
                    Application.DoEvents()
                End If
...
#Region " Private Methods "
    ...
    Private Function Processfile_Check(ByVal sfile As String, ByVal oClient As Clients, ByVal FileDate As Date) As Boolean
        ...
        If sCSV.GetUpperBound(0) > sTSV.GetUpperBound(0) Then
            If (InStr(sfile, "GAVD_") > 0) Or (InStr(sfile, "GAVM_") > 0) Then
                Return ProcessfileV_CSV(sfile, oClient, FileDate)
            Else
    ...
    Private Function ParseColumns(ByVal sLine As String) As String()
        ...
        Return sColumns
    End Function
    Private Function ParseColumnsV(ByVal sLine As String) As String()
        Dim sColumns() As String = sLine.Split(",")
        Dim nCount As Integer
        'Find Date Column
        nColumnDate = -1
        For nCount = sColumns.GetLowerBound(0) To sColumns.GetUpperBound(0)
            If sColumns(nCount).ToLower = "date" Then
                ...
                sColumns(nCount) = ""
            Else
                sColumns(nCount) = _ColumnsV(sColumns(nCount))
            End If
        Next
...
#Region " CheckProcessed "
    Public Function CheckNotProcessed(ByVal Clients() As Clients, ByVal CheckDate As Date, ByVal bCheckProcessedDir As Boolean) As String()
        ...
        sBad(0) = "(GA) Date: " & CheckDate.ToString("yyyy_MM_dd")
        ...
        For Each client As Clients In Clients
            If client.GA.Has_AccountID Then
                ...
                Dim sFileName1 As String = sDir & "GA_" & Replace(client.Name, " ", "") & CheckDate.ToString("_yyyy_MM_dd") & ".csv"
                Dim sFileName2 As String = sDir & "GAD_" & Replace(client.Name, " ", "") & CheckDate.ToString("_yyyy_MM_dd") & ".csv"
                Dim sFileName3 As String = sDir & "GAM_" & Replace(client.Name, " ", "") & CheckDate.ToString("_yyyy_MM_dd") & ".csv"
                ...
                If bExists = False Then
                    ...
                    sBad(Index) = "GA MISSING FOR (" & client.CustID.ToString & ") " & client.Name
                End If
            End If
        Next
